...
*	                                                                      *
*	created: 08/22/2005	              geaendert am: 10/16/2005        *
*									      *
*			Version: 1.1					      *
*	                                                                      *
\*---------------------------------------------------------------------------*/
...
- initFromDeviceDescription:devDesc
{
    ...
    lp->dd = (Vmxnet2_DriverData *)
    		(((unsigned long)IOMalloc(driverDataSize + 15) + 15) & ~15);
...
- receivePackets
{
	...
	{
	    netbuf_t	newskb=nb_alloc(PKT_BUF_SZ);
	    
	    if (newskb != NULL)
	    {
		netbuf_t	*rxSkbuff=(netbuf_t*)rre->driverData;

		skb = *rxSkbuff;
		nb_shrink_bot(skb, nb_size(skb));
		nb_grow_bot(skb, pkt_len);

		*rxSkbuff = newskb;	    
		irtn = IOPhysicalFromVirtual(IOVmTaskSelf(), 
				(vm_address_t)nb_map(newskb), 
				&rre->paddr);
...
- (Vmxnet_TxStatus)doTransmit:(netbuf_t)skb :(Bool)isCOS
{
	...
	irtn = IOPhysicalFromVirtual(IOVmTaskSelf(), (vm_address_t)nb_map(skb), 
			&xre->sg.sg[0].addrLow);
	if (irtn = IO_R_SUCCESS)
	    IOLog("%s: Error getting paddr: %d\n", [self name], irtn);
...
- initRing
{
    ...
    {
	lp->rxSkbuff[i] = nb_alloc(PKT_BUF_SZ);
	if (lp->rxSkbuff[i] == NULL)
	{
				/* there is not much we can do at this point */
	    IOLog("%s: vmxnet_init_ring nb_alloc failed.\n", [self name]);
	    return nil;
	} /* if */

	irtn = IOPhysicalFromVirtual(IOVmTaskSelf(), 
			(vm_address_t)nb_map(lp->rxSkbuff[i]), 
			&lp->rxRing[i].paddr);
	if (irtn = IO_R_SUCCESS)
	    IOLog("%s: Error getting paddr: %d\n", [self name], irtn);
...
- checkTxQueue
{
	...
	nb_free((netbuf_t)xre->driverData);
	xre->driverData = NULL;
...
- close
{
	...
	{
	    nb_free((netbuf_t)lp->txRing[i].driverData);
	    lp->txRing[i].driverData = NULL;
	} /* if */
    } /* for */
